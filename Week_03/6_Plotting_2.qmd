...
![](https://i.pinimg.com/originals/ea/0c/b5/ea0cb5a48e3337523ffe8d60f177879d.gif){fig-align="center" width="55%"}
...
Using `scale_color_viridis_d()` viridis palettes are designed to be **perceptually uniform and distinguishable under common forms of color blindness**. This connects back to the "responsible plotting" idea from our first lecture: color choices are part of communicating data honestly, not just aesthetics.
...
First, you need to install the *{devtools}* package (Development tools) and *{pak}*
...
Instead of using *install.packages("PackageName")*, we used to use *install_github("username/packagename")*, which will still work but is now old. You will see this example often on github

**Modern alternative:** The `pak` package is newer lighter-weight and handles dependencies better:
```{r, eval=FALSE, echo=TRUE}
install.packages("pak")
pak::pkg_install("username/packagename")  # same idea, more robust
```
:::
...
::: {.center}
![](libs/Images/Beyonce.PNG){fig-align="center" width="65%"}
:::
:::
...
- coord_flip(): Cartesian coordinate system with x and y axes flipped.  
  *(Note: modern ggplot2 practice prefers **swapping x and y in `aes()` directly** rather than using `coord_flip()` — it is simpler and avoids unexpected behavior with some geoms.)*
...
::: {.center}
[There are hundreds of ggplot2 extensions](https://exts.ggplot2.tidyverse.org/gallery/)  
[Great resource for ggplot2](https://ggplot2-book.org/index.html)
:::

A few you'll likely find useful:

| Package | What it does |
|---|---|
| `patchwork` | Combine multiple plots into one figure |
| `ggrepel` | Auto-position point/text labels so they don't overlap |
| `gganimate` | Animate ggplot2 graphics |
| `ggtext` | Rich text (markdown/HTML) in titles, labels, and annotations |
